...
public class PlayerPiano {
        ...
        throws MidiUnavailableException, InvalidMidiDataException, IOException
    {
        ...
        arr[0] = "";
        new_notes = sb.toString().split("[()]"); 
        for (int i =0; i<new_notes.length ;i++)
        {
        	arr[0] = arr[0]+new_notes[i]+" ";
        } 
        int a = 0; 
        while(a < arr.length) {
            if (arr[a].equals("-i")) {
                instrument = Integer.parseInt(arr[a+1]);
                a+=2;
            }
            else if (arr[a].equals("-t")) {
                tempo = Integer.parseInt(arr[a+1]);
                a+=2;
            }
            else if (arr[a].equals("-o")) {
                filename = arr[a+1];
                a += 2;
            }
            else break;
